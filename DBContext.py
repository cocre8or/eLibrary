...
class DBContext:
  ...
  def CreateUser(self, username, pwd, email_address):
    try:
      ...
      if len(results) > 0:
        ...
        result = self.eLibraryUsers.find({"token": results[2]}, {"_id": 0, "token": 1})
        return dumps(result)
      else:
        return None
    except:
  ...
  def viewBooks(self):
    return self.Parse(self.eLibraryBooks.find({}, {"_id": 0}))
  ...
  def checkOut(self, bookTitle, cardId):
    try:
      ...
      return self.Parse(self.eLibraryBooks.find({"title": bookTitle}, {"_id": 0}))
    except:
      return None
